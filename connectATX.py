...
class ConnectATX(QObject):
    ...
    def run(self):
        try:
            ...
            while True:
                ...
                Match = self.find_pattern3(lower_image, pattern)
                if Match is None:
                    continue
        ...
        for i in range(3):  # 3개의 줄을 읽기 위해 3번 반복
            ...
            for j in range(6):  # 각 줄에서 6개 좌표를 읽습니다.
                ...
                for color_name, (rgb, touch_value) in color_map.items():
                    ...
                    if all(abs(rv - ev) <= 4 for rv, ev in zip(rgb_value, expected_rgb)):
                        ...
                        if Match == 'RedSet':
                            ...
                            if any(key in color_name for key in ['Red', 'Orange', 'Yellow', 'Purple']):
                                row_points.append(touch_value)  # 해당 좌표를 추가
                                matched = True
                                break
                            else:
                                row_points.append('-')  # Set에 속하지 않으면 '-'
                        elif Match == 'GreenSet':
            ...
            # 연속된 '-' 값 병합
            row_points = self.merge_hyphens(row_points)

            # 매칭된 좌표를 저장
            upper_touch_points.extend(row_points)

        return upper_touch_points, matched


    def merge_hyphens(self, points):
        # 연속된 '-' 값을 하나로 병합
        merged_points = []
    ...
    def find_pattern3(self, image, pattern):
        # 색상별 좌표와 RGB 값 설정
        ...
        if max_val >= threshold:
            print("GreenSet 반환: 패턴 이미지가 발견되었습니다.")
            return 'GreenSet'
        else:
            print("RedSet 반환: 패턴 이미지가 발견되지 않았습니다.")
            return 'RedSet'
    ...
    def find_matching_points(self, upper_touch_points):
        for point in upper_touch_points:
            if point == '-':
